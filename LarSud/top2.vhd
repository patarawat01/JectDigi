...
architecture Behavioral of task_scheduler_top_v2 is
    ...
    signal byte_phase      : integer range 0 to 2 := 0;
    ...
    signal task_4bit       : std_logic_vector(3 downto 0);

    signal servo_slot_reg  : unsigned(1 downto 0) := "01";
...
begin
    ...
    task_4bit <= std_logic_vector(resize(current_task, 4));
    ...
    -- Servo
    ----------------------------------------------------------------
    U_SERVO : entity work.servo_pwm
        port map (
            clk      => clk,
            rst      => rst,
            enable   => '1',
            position => servo_slot_reg,
            pwm_out  => servo_sig
        );

    ----------------------------------------------------------------
    -- Start button: Basys3 button is active HIGH
    ----------------------------------------------------------------
    process(clk, rst)
    begin
        if rst = '1' then
    ...
    begin
        if rst = '1' then
            ...
            case state_reg is
                when LOAD_TASKS | WAIT_START | FINISH =>
                    servo_slot_reg <= "01";

                when SELECT_TASK =>
                    if selected_valid = '1' then
                        servo_slot_reg <= selected_task;
                    end if;

                when LOAD_TASK | RUN_TASK | MARK_DONE =>
                    servo_slot_reg <= current_task;
    ...
    begin
        if rst = '1' then
            ...
            case state_reg is
                ...
                when LOAD_TASK =>
                    load_value_sig <= task_durations(to_integer(current_task));
                    load_en_sig    <= '1';
                    state_reg      <= RUN_TASK;
                ...
                when MARK_DONE =>
                    done_bits(to_integer(current_task)) <= '1';
                    state_reg <= SELECT_TASK;
    ...
    ext_led <= blink_reg when state_reg = SELECT_TASK or
                          state_reg = LOAD_TASK or
                          state_reg = RUN_TASK
          else '0';
