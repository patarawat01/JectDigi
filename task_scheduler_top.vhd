...
begin
    ...
    U_COUNTER : entity work.run_counter
        port map (
            clk        => clk,
            rst        => rst,
            tick_en    => tick_sig when state_reg = RUN_TASK else '0',
            load_en    => load_en_sig,
    ...
    begin
        if rst = '1' then
            ...
            case state_reg is
                ...
                when RUN_TASK =>
                    -- counter_zero จะเป็น 1 หลังนับถึง 0
    ...
    ----------------------------------------------------------------
    -- LED Output
    -- leds(3 downto 0)   = current running task (one-hot)
    -- leds(7 downto 4)   = done bits
    -- leds(11 downto 8)  = remaining counter
    -- leds(14 downto 12) = state code
    -- leds(15)           = finish flag
    ----------------------------------------------------------------
